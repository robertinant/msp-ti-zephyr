#define DT_DRV_COMPAT virtual_lin_mspm0
...
/* private types */
enum lin_mspm0_state {
    LIN_MSPM0_STATE_IDLE,
    LIN_MSPM0_STATE_BREAK,
    LIN_MSPM0_STATE_SYNC,
    LIN_MSPM0_STATE_PID,
    LIN_MSPM0_STATE_SEND_RESPONSE,
    LIN_MSPM0_STATE_SEND_CHECKSUM,
    LIN_MSPM0_STATE_RECV_RESPONSE,
};
...
/* private helpers */
static void lin_mspm0_send_break(const struct device *dev) {
    ...
    data->state = LIN_MSPM0_STATE_BREAK;
    k_timer_start(&data->timer, data->break_timeout, K_FOREVER);
    uart_poll_out(cfg->uart_dev, BREAK);
}

static void lin_mspm0_send_byte(const struct device *dev, uint8_t c) {
    const struct lin_mspm0_config *cfg = dev->config;
    struct lin_mspm0_data *data = dev->data;
 ...
 */
static void lin_mspm0_set_nominal_bitrate(const struct device *dev) {
    const struct lin_mspm0_config *cfg = dev->config;
    struct lin_mspm0_data *data = dev->data;
...
static void lin_mspm0_clear_isr(const struct device *dev) {
    const struct lin_mspm0_config *cfg = dev->config;
...
static void lin_mspm0_release_isr(struct lin_mspm0_data *data) {
    data->irq_has_lock = false;
    k_sem_give(&data->lock);
}

static void lin_mspm0_update_checksum(struct lin_mspm0_data *data, uint8_t c) {
...
static void lin_mspm0_process_fail(const struct device *dev) {
    struct lin_mspm0_data *data = dev->data;
    if (data->mode == LIN_MODE_COMMANDER) {
        if (data->state == LIN_MSPM0_STATE_BREAK) {
            lin_mspm0_set_nominal_bitrate(dev);
        }
        ...
        data->state = LIN_MSPM0_STATE_IDLE;
    } else { // LIN_MODE_RESPONDER
        if (data->callbacks.tx && (data->state == LIN_MSPM0_STATE_SEND_RESPONSE ||
            data->state == LIN_MSPM0_STATE_SEND_CHECKSUM)) {
            data->callbacks.tx(dev, -EIO, data->callbacks.tx_data);
        } else if (data->callbacks.rx && data->state == LIN_MSPM0_STATE_RECV_RESPONSE) {
            data->callbacks.rx(dev, -EIO, NULL, data->callbacks.rx_data);
        }
        data->state = LIN_MSPM0_STATE_BREAK;
    }
...
static void lin_mspm0_irq_handler(const struct device *uart_dev, void *lin_dev) {
    const struct device *dev = lin_dev;
    struct lin_mspm0_data *data = dev->data;
    if (k_sem_take(&data->irq_lock, K_NO_WAIT)) {
        lin_mspm0_clear_isr(dev);
    ...
    while (uart_irq_update(uart_dev) && uart_irq_is_pending(uart_dev)) {
        ...
        if (err > 0) {
            ...
            if (data->mode == LIN_MODE_RESPONDER && (err & (UART_ERROR_FRAMING | UART_BREAK))) {
                lin_mspm0_process_fail(dev); // call callbacks if packet was in progress
                k_timer_start(&data->timer, data->sync_timeout, K_FOREVER);
                data->state = LIN_MSPM0_STATE_SYNC;
            } else {
                LOG_ERR("unexpected error byte %d", err);
                goto lin_mspm0_irq_fail;
            }
        }
        ...
        if (uart_irq_rx_ready(uart_dev)) {
            ...
            if (data->mode == LIN_MODE_COMMANDER) {
                switch (data->state) {
                    case LIN_MSPM0_STATE_BREAK:
                        if (bite != BREAK) {
                            LOG_ERR("break mismatch 0x%x", bite);
                            goto lin_mspm0_irq_fail;
                        }
                        lin_mspm0_set_nominal_bitrate(dev);
                        lin_mspm0_send_byte(dev, SYNC);
                        data->state = LIN_MSPM0_STATE_SYNC;
                        break;

                    case LIN_MSPM0_STATE_SYNC:
                        if (bite != SYNC) {
                            LOG_ERR("sync mismatch 0x%x", bite);
                            goto lin_mspm0_irq_fail;
                        }
                        lin_mspm0_send_byte(dev, PID[data->frame.id]);
                        data->state = LIN_MSPM0_STATE_PID;
                        break;

                    case LIN_MSPM0_STATE_PID:
                        ...
                        if (data->sending) {
                            ...
                            lin_mspm0_send_byte(dev, data->frame.data[0]);
                            lin_mspm0_update_checksum(data, data->frame.data[0]);
                            data->state = LIN_MSPM0_STATE_SEND_RESPONSE;
                        } else {
                            data->checksum = 0;
                            data->state = LIN_MSPM0_STATE_RECV_RESPONSE;
                            k_timer_start(&data->timer, data->timeout, K_FOREVER);
                        }
                        break;

                    case LIN_MSPM0_STATE_SEND_RESPONSE:
                    case LIN_MSPM0_STATE_SEND_CHECKSUM:
                    case LIN_MSPM0_STATE_RECV_RESPONSE:
                        processed = false;
                        break;
                ...
                switch (data->state) {
                    ...
                    case LIN_MSPM0_STATE_PID:
                        ...
                        switch (action) {
                            ...
                            default:
                                lin_mspm0_release_isr(data);
                                data->state = LIN_MSPM0_STATE_BREAK; // spurious errors will be logged
                                break;
                        }
                        break;
                ...
                switch (data->state) {
                    case LIN_MSPM0_STATE_SEND_RESPONSE:
                        ...
                        if (data->frame_idx < data->frame.len) {
                            lin_mspm0_send_byte(dev, data->frame.data[data->frame_idx]);
                            lin_mspm0_update_checksum(data, data->frame.data[data->frame_idx]);
                        } else {
                            lin_mspm0_send_byte(dev, ~data->checksum & 0xFF);
                            data->state = LIN_MSPM0_STATE_SEND_CHECKSUM;
                        }
                        break;
                    ...
                    case LIN_MSPM0_STATE_RECV_RESPONSE:
                        ...
                        lin_mspm0_update_checksum(data, bite);
...
lin_mspm0_irq_fail:
    lin_mspm0_clear_isr(dev);
    lin_mspm0_process_fail(dev);
    lin_mspm0_release_isr(data);
    k_sem_give(&data->irq_lock);
}

static void lin_mspm0_timeout_handler(struct k_timer *timer) {
    struct lin_mspm0_data *data = CONTAINER_OF(timer, struct lin_mspm0_data, timer);
    if (k_sem_take(&data->irq_lock, K_NO_WAIT)) {
        return; // uart isr is handling already
    }
    LOG_ERR("frame timeout in state %d", data->state);
    lin_mspm0_process_fail(data->lin_dev);
    lin_mspm0_release_isr(data);
    k_sem_give(&data->irq_lock);
}
...
static int lin_mspm0_set_header_callback(const struct device *dev, lin_header_callback_t callback, void *user_data) {
    struct lin_mspm0_data *data = dev->data;
    k_sem_take(&data->lock, K_FOREVER);
...
static int lin_mspm0_set_tx_callback(const struct device *dev, lin_tx_callback_t callback, void *user_data) {
    struct lin_mspm0_data *data = dev->data;
    k_sem_take(&data->lock, K_FOREVER);
...
static int lin_mspm0_set_rx_callback(const struct device *dev, lin_rx_callback_t callback, void *user_data) {
    struct lin_mspm0_data *data = dev->data;
    k_sem_take(&data->lock, K_FOREVER);
...
static int lin_mspm0_receive(const struct device *dev, uint8_t id, enum lin_checksum type, uint8_t len) {
...
static int lin_mspm0_init(const struct device *dev) {
    ...
    k_timer_init(&data->timer, lin_mspm0_timeout_handler, NULL);

    if (lin_mspm0_set_bitrate(dev, data->uart_cfg.baudrate)) {
        return -EINVAL;
    }

    uart_irq_callback_user_data_set(cfg->uart_dev, lin_mspm0_irq_handler, (void*) dev);
